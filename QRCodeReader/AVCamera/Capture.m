...
- (instancetype)init {
    _sessionQueue = dispatch_queue_create("com.hiscene.jt.captureSesstionQueue", DISPATCH_QUEUE_SERIAL);
...
- (instancetype)initWithSessionPreset:(NSString *)sessionPreset
    ...
    if (self) {
        ...
        _setupResult = CaptureSetupResultSuccess;
        ...
        switch ( [AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo] ) {
            ...
            case AVAuthorizationStatusNotDetermined: {
                ...
                [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^( BOOL granted ) {
                    if ( ! granted ) {
                        _setupResult = CaptureSetupResultCameraNotAuthorized;
                    }
...
- (void)start {
    dispatch_async( self.sessionQueue, ^{
        if ( _setupResult == CaptureSetupResultSuccess ) {
...
- (void)setFlashMode:(AVCaptureFlashMode)flashMode {
    dispatch_async( self.sessionQueue, ^{
        if (_flashMode != flashMode) {
            ...
            if ( _setupResult != CaptureSetupResultSuccess) return;
...
- (void)configureSession {
    ...
    if ([AVCaptureDevice instancesRespondToSelector:@selector(defaultDeviceWithDeviceType:mediaType:position:)]) {
...
#endif
        _setupResult = CaptureSetupResultSessionConfigurationFailed;
